...
void turnleft(bool debug = false){ 
  forward();
  delay(500);
  left();
  delay(400);
...
void turnright(bool debug = false){ 
  forward();
  delay(500);
  right();
  delay(400);
...
void getBTData() {
  if(Serial.available()) {
    switch(Serial.read()) {
      ...
      case '3': func_mode = Bluetooth; mov_mode = TLEFT;    break;
      case '4': func_mode = Bluetooth; mov_mode = TRIGHT;   break;
